...
- **Key format & storage:** raw key is `cos_live_<32 random bytes, base62>`, shown exactly once in the `POST` response; `key_hash` is SHA-256 of the raw key, not Argon2id (FR-PLAT-2's password hashing) — password auth always looks a user up by email *before* verifying, but an incoming API request carries only the raw key itself, so authenticating it needs a direct `WHERE key_hash = …` lookup. That's safe here specifically because a generated key already has ≥192 bits of entropy: unlike a human-chosen password, it's not brute-forceable or rainbow-table-able regardless of hash speed, so the slow/salted properties Argon2id exists for aren't needed, and a fast deterministic hash is what makes the lookup possible at all. A lost key cannot be recovered — only revoked and replaced.
