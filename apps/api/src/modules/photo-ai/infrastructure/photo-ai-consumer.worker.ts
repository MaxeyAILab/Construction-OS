...
const DURABLE_NAME = "photo-ai-tagger";
const MAX_DELIVERY_ATTEMPTS = 5;
const jsonCodec = JSONCodec();

// ai-spec.md §7.8: its own durable JetStream pull consumer, independent
// of every other event consumer in this app — same "each fans out
// independently, one falling behind never affects the others" reasoning
// as RagIndexingConsumerWorker (byte-for-byte the same structure).
@Injectable()
export class PhotoAiConsumerWorker implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(PhotoAiConsumerWorker.name);
  private messages?: ConsumerMessages;
  private loopPromise?: Promise<void>;

  constructor(
    @Inject(NATS_CONNECTION) private readonly nc: NatsConnection,
    private readonly writer: PhotoAiWriterService,
  ) {}
