...
const DURABLE_NAME = "equipment-fault-alerts";
const MAX_DELIVERY_ATTEMPTS = 5;
const jsonCodec = JSONCodec();

// FR-EQ-4: its own durable JetStream pull consumer, independent of every
...
// FinanceAlertsConsumerWorker/RagIndexingConsumerWorker (byte-for-byte the
// same structure).
@Injectable()
export class EquipmentFaultAlertsConsumerWorker implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(EquipmentFaultAlertsConsumerWorker.name);
  private messages?: ConsumerMessages;
  private loopPromise?: Promise<void>;

  constructor(
    @Inject(NATS_CONNECTION) private readonly nc: NatsConnection,
    private readonly writer: EquipmentFaultAlertsWriterService,
  ) {}
