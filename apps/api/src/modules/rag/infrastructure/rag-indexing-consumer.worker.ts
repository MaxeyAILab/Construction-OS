...
const DURABLE_NAME = "rag-indexing-writer";
const MAX_DELIVERY_ATTEMPTS = 5;
const jsonCodec = JSONCodec();

// ai-spec.md §3: "outbox event → embedding worker" — its own durable
// JetStream pull consumer, independent of audit's/notifications'/
// dashboard-projections' own consumers, same "each fans out
// independently, one falling behind never affects the others" reasoning
// as every other event consumer in this app.
@Injectable()
export class RagIndexingConsumerWorker implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(RagIndexingConsumerWorker.name);
  private messages?: ConsumerMessages;
  private loopPromise?: Promise<void>;

  constructor(
    @Inject(NATS_CONNECTION) private readonly nc: NatsConnection,
    private readonly writer: RagIndexingWriterService,
  ) {}
